...
class DFMODELS():
    ...
    def denfeng_model(self):
        ...
        """
        result_orinignal = self.one_lm_modle(self.data_x, self.data_y, "modelorinagnal")
        for i in range(0, self.K_alpha):  #表示有几个模型
            modelname = "model" + str(i)  # 定义新的模型的名称
            data_x_temp = self.data_x.ix[i*self.delta:(i + 1)*self.delta-1, :]
            data_y_temp = self.data_y[i*self.delta:(i + 1)*self.delta]
            result_temp = self.one_lm_modle(data_x_temp, data_y_temp, modelname)
            result_orinignal = result_orinignal.append(result_temp)
        print(result_orinignal)
        result_orinignal.to_csv(self.savefilenames)
    ...
    def DFMODELplot(self,result_Dengfen,x11,x12,x13,x14):
        ...
        for modelx in range(0, X):
            ...
            def magnifi(delta,c,y,x):
                result=[]
                for i in range(delta): #分成了三段 每一段的系数不同 因此每一段需要乘以不同的系数
                    for j in range(i*self.delta,(i+1)*self.delta):
                        temp=0
                        if x11!=0:
                            temp+=c[i]*x[j]
                        if x12!=0:
                        ...
                        if x13!=0:
                            temp+=y[i]*x[j]*x[j]
                        if x14!=0:
                            temp+=y[i]*x[j]+x[j]+x[j]
                        result.append(temp)
                return result

            up_config_x=magnifi(self.K_alpha,constant_x_1,up_config_x_1,x_label)
            downconfig_x=magnifi(self.K_alpha,constant_x_1,downconfig_x_1,x_label)
            coef_x=magnifi(self.K_alpha,constant_x_1,coef_x_1,x_label)
            ...
            plt.hlines(ori_coef_x, 1, self.N + 1, colors="red")
            plt.hlines(ori_up_x, 1, self.N + 1, colors="green", linestyles='--')
            plt.hlines(ori_down_x, 1, self.N + 1, colors="green", linestyles='--')
